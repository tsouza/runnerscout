#!/usr/bin/env bash
# Top-level orchestrator for the full real AWS end-to-end qualification
# (issue #80): bring-up -> dispatch-and-wait -> guaranteed teardown, with
# teardown always attempted via `trap ... EXIT`, exactly mirroring
# qualify-aws.yml's `if: always()` final step - the local-runbook
# equivalent of a workflow-level guarantee, since no GitHub Actions runtime
# is enforcing this for us here. See docs/e2e-qualification.md for the full
# harness, its safety bounds, and every environment variable each stage
# reads; see docs/e2e-qualification.background.md for why this harness is a
# local runbook rather than a GitHub Actions workflow.
#
# This script never registers a scale set itself - run
# tools/e2e/register-scale-set.sh first and export its printed ID as
# E2E_REGISTERED_SCALE_SET_ID. Keeping registration a separate, explicit
# step means a single `run.sh` invocation can never silently create a new
# GitHub-side scale set as a side effect of an unrelated retry.
#
# Usage: tools/e2e/run.sh
...
tools/e2e/bring-up.sh

e2e_log "=== stage 2/2: dispatch and wait ==="
tools/e2e/dispatch-and-wait.sh
