...
# qualify-aws.yml/qualify-azure.yml enforce before they ever touch a real
# credential: e2e_require_confirmation (the exact-phrase spend confirmation)
# and e2e_require_ceiling (a numeric runtime bound no input can raise past a
...
# aws_realcloud_test.go's/azure_realcloud_test.go's requireRealCloudConfirmation
# is independent of qualify-aws.yml's/qualify-azure.yml's own workflow-level
...
# qualify-azure.yml: a hard-coded ceiling no E2E_MAX_RUNTIME_MINUTES value
# can exceed. This harness does strictly more than those workflows (cluster
# bring-up, real scale-set registration, a real dispatched job, real runner
...
# qualify-aws.yml's/qualify-azure.yml's own "recompute from inputs/env
# directly" discipline.
e2e_require_ceiling() {
...
# by every bring-up/dispatch-and-wait script (any cloud) before any
# e2e_render call. Cloud-specific image/architecture/machine-type identifiers
# are deliberately NOT defaulted here: they have no safe default (an
# operator-chosen image/architecture pair must match exactly across bring-up
# and every later catalog refresh), so each cloud's own script requires them
# explicitly via e2e_require_var instead.
...
# actually present in the template being rendered, so listing e.g. Azure
# names here is harmless for an AWS template and vice versa.
e2e_render() {
