...
#
# Run this only after tools/e2e/bring-up.sh reports its RunnerScaleSet
...
#   E2E_NAMESPACE, E2E_SCALE_SET_NAME                 must match bring-up.sh
#   E2E_AWS_REGION, E2E_AWS_ACCOUNT_ID, E2E_AWS_VPC_ID, E2E_AWS_SUBNET_ID,
#   E2E_AWS_SECURITY_GROUP_ID, E2E_AWS_INSTANCE_TYPE   for the catalog refresh
...
# Same full set of resource-graph inputs bring-up.sh requires for the
# CapacityCatalog it renders - this script re-renders that same manifest
# (see manifests/capacity-catalog.yaml.tmpl's header comment for why) as a
# separate process, so it needs every variable that manifest references,
# not only the ones this script's own AWS cross-validation logic touches.
for v in E2E_SCALE_SET_NAME E2E_AWS_REGION E2E_AWS_ACCOUNT_ID E2E_AWS_VPC_ID \
         E2E_AWS_SUBNET_ID E2E_AWS_SECURITY_GROUP_ID E2E_AWS_INSTANCE_TYPE \
         E2E_AWS_AMI_ID E2E_AWS_ARCHITECTURE \
...
e2e_render capacity-catalog.yaml.tmpl "$work_dir/capacity-catalog.yaml"
kubectl apply -n "$E2E_NAMESPACE" -f "$work_dir/capacity-catalog.yaml"
...
if [ -n "$allocation_id" ]; then
  ...
  while true; do
    ...
    if [ "$(date +%s)" -ge "$deadline" ]; then
      e2e_log "::warning:: allocation $allocation_id still in phase=$phase at the runtime ceiling - tools/e2e/verify.sh and tools/e2e/teardown.sh will still independently confirm/force cloud-side cleanup"
      break
    fi
    sleep 5
  done
