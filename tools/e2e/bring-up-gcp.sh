...
# wired to one real GCP ProviderConfig/RunnerClass/RunnerScaleSet/
# CapacityCatalog/NetworkProfile resource graph - modeled closely on
# examples/multicloud and on tools/e2e/bring-up.sh (the AWS equivalent),
# GCP-only, real network identifiers as inputs, and a real,
...
# tools/e2e/dispatch-and-wait-gcp.sh (run this after). Splitting bring-up
...
#   E2E_GCP_SUBNETWORK, E2E_GCP_IMAGE, E2E_GCP_MACHINE_TYPE,
#   E2E_GCP_CREDENTIALS_FILE
# Optional (defaults shown):
#   E2E_NAMESPACE=runnerscout  E2E_CLUSTER_NAME=runnerscout-e2e
#   E2E_IMAGE=runnerscout:e2e  E2E_GCP_OFFERING_CPU=2
#   E2E_GCP_OFFERING_MEMORY_MIB=4096  E2E_GCP_ARCHITECTURE=amd64
#   E2E_PROVISIONING_SECONDS=300  E2E_MAX_LIFETIME_SECONDS=1800
#   E2E_GCP_MAX_PRICE_MICROS=200000  E2E_GCP_PRICE_MICROS=10000 (static, see
#     manifests/capacity-catalog-gcp.yaml.tmpl - never a real observation)
...
#     mechanics smoke test, which never needs real GCP/GitHub credentials)
set -euo pipefail
...
for v in E2E_GITHUB_URL E2E_SCALE_SET_NAME E2E_REGISTERED_SCALE_SET_ID E2E_GITHUB_AUTH_MODE \
         E2E_GCP_PROJECT E2E_GCP_REGION E2E_GCP_ZONE E2E_GCP_NETWORK \
         E2E_GCP_SUBNETWORK E2E_GCP_IMAGE E2E_GCP_MACHINE_TYPE; do
  e2e_require_var "$v"
done
...
if [ -z "${E2E_SKIP_IMAGE_BUILD:-}" ]; then
  e2e_require_var E2E_GCP_CREDENTIALS_FILE
fi
...
e2e_log "cross-validating pinned GCP identifiers (read-only)"
if [ -z "${E2E_SKIP_IMAGE_BUILD:-}" ]; then
  e2e_refresh_gcp_catalog_inputs
else
  e2e_log "E2E_SKIP_IMAGE_BUILD set: mechanics smoke test mode, skipping real GCP calls; using placeholder catalog values"
  export E2E_GCP_SUBNET_CIDR="10.98.0.0/24"
  observed_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  export E2E_GCP_PRICE_OBSERVED_AT="$observed_at"
fi
...
e2e_log "creating GitHub and GCP credential Secrets"
case "$E2E_GITHUB_AUTH_MODE" in
  pat)
...
if [ -n "${E2E_SKIP_IMAGE_BUILD:-}" ] && [ -z "${E2E_GCP_CREDENTIALS_FILE:-}" ]; then
  # A minimal, well-formed (but useless) authorized_user-shaped credential
  # document - just enough for gcpCredentialDocument's own shape checks
  # (internal/provider/gcp_credentials.go) to accept it as a file the mount
  # mechanics can carry end-to-end; it is never used to reach real GCP in
  # this mode.
  cat > "$work_dir/gcp-credentials-placeholder" <<'EOF'
{"type": "authorized_user", "client_id": "placeholder", "client_secret": "placeholder", "refresh_token": "placeholder"}
EOF
  E2E_GCP_CREDENTIALS_FILE="$work_dir/gcp-credentials-placeholder"
fi
kubectl -n "$E2E_NAMESPACE" create secret generic gcp-credentials \
  --from-file=credentials="$E2E_GCP_CREDENTIALS_FILE" \
  --from-literal=path=/etc/runnerscout/providers/gcp-credentials/credentials \
  --dry-run=client -o yaml | kubectl apply -f -

e2e_log "rendering and applying the resource graph"
render provider-config-gcp.yaml.tmpl
render network-profile-gcp.yaml.tmpl
render capacity-catalog-gcp.yaml.tmpl
render runner-class-gcp.yaml.tmpl
render "runner-scaleset-${E2E_GITHUB_AUTH_MODE}-gcp.yaml.tmpl"
kubectl apply -n "$E2E_NAMESPACE" \
  -f "$work_dir/provider-config-gcp.yaml.tmpl" \
  -f "$work_dir/network-profile-gcp.yaml.tmpl" \
  -f "$work_dir/capacity-catalog-gcp.yaml.tmpl" \
  -f "$work_dir/runner-class-gcp.yaml.tmpl" \
  -f "$work_dir/runner-scaleset-${E2E_GITHUB_AUTH_MODE}-gcp.yaml.tmpl"
...
helm upgrade --install runnerscout charts/runnerscout --namespace "$E2E_NAMESPACE" \
  ...
  --set crd.secretNames[1]=gcp-credentials \
  --set credentialSecrets[0].name=gcp-credentials

if [ -n "${E2E_SKIP_IMAGE_BUILD:-}" ]; then
  e2e_log "mechanics smoke test: cluster/CRDs/chart installed; not waiting for a real Ready condition (no real GitHub/GCP credentials configured)"
  exit 0
fi
