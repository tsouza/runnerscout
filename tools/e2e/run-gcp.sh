#!/usr/bin/env bash
# Top-level orchestrator for the full real GCP end-to-end qualification
# (issue #80): bring-up -> dispatch-and-wait -> guaranteed teardown, with
# teardown always attempted via `trap ... EXIT`, exactly mirroring
# qualify-gcp.yml's `if: always()` final step and tools/e2e/run.sh (the AWS
# equivalent) - the local-runbook equivalent of a workflow-level guarantee,
# since no GitHub Actions runtime is enforcing this for us here. See
# docs/e2e-qualification.md for the full harness, its safety bounds, and
# every environment variable each stage reads; see
...
# step means a single `run-gcp.sh` invocation can never silently create a
# new GitHub-side scale set as a side effect of an unrelated retry.
#
# Usage: tools/e2e/run-gcp.sh
...
tools/e2e/bring-up-gcp.sh

e2e_log "=== stage 2/2: dispatch and wait ==="
tools/e2e/dispatch-and-wait-gcp.sh
