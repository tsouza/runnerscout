...
# E2E_MAX_RUNTIME_MINUTES ceiling elapses - whichever comes first. The
# Azure sibling of tools/e2e/dispatch-and-wait.sh (AWS).
#
# This script is NOT reused verbatim from the AWS piece: it re-renders
# manifests/capacity-catalog-azure.yaml.tmpl (a different file, with a
# different variable set) via e2e_refresh_azure_catalog_inputs rather than
# e2e_refresh_aws_catalog_inputs - see that AWS script's own header comment,
# read in full before writing this one, which confirmed neither the
# dispatch/poll loop below nor the fleet-ConfigMap logic is cloud-specific
# (both are copied here unchanged) but the catalog-refresh call and its
# required-variable list are.
#
# Run this only after tools/e2e/bring-up-azure.sh reports its
# RunnerScaleSet Ready. This script uses the operator's own `gh` session
# against E2E_TARGET_REPO - deliberately NOT this repository's gh-tsouza
# wrapper (see AGENTS.md): gh-tsouza is scoped to this coordinating
# session's own operations against tsouza/runnerscout, while
# E2E_TARGET_REPO is an operator-chosen, unrelated test repository whose
# credentials/account this harness has no opinion about. Run `gh auth
# status` yourself first. It also uses the operator's own authenticated
# `az` CLI session for the pre-dispatch catalog refresh's read-only
# cross-validation - run `az login` (or equivalent) yourself first, exactly
# like tools/e2e/bring-up-azure.sh assumes.
#
# Required environment (see docs/e2e-qualification.md):
#   E2E_CONFIRM_REAL_SPEND, E2E_MAX_RUNTIME_MINUTES   tools/e2e/lib.sh gates
#   E2E_NAMESPACE, E2E_SCALE_SET_NAME                 must match bring-up-azure.sh
...
#   E2E_AZURE_ARCHITECTURE                            for the catalog refresh
...
# Same full set of resource-graph inputs bring-up-azure.sh requires for the
# CapacityCatalog it renders - this script re-renders that same manifest
# (see manifests/capacity-catalog-azure.yaml.tmpl's header comment for why)
# as a separate process, so it needs every variable that manifest
# references, not only the ones this script's own Azure cross-validation
# logic touches.
for v in E2E_SCALE_SET_NAME E2E_AZURE_REGION E2E_AZURE_SUBSCRIPTION_ID E2E_AZURE_RESOURCE_GROUP \
         E2E_AZURE_VNET_ID E2E_AZURE_SUBNET_ID E2E_AZURE_NSG_ID E2E_AZURE_IMAGE_ID \
         E2E_AZURE_VM_SIZE E2E_AZURE_ZONE E2E_AZURE_ARCHITECTURE \
...
e2e_log "refreshing CapacityCatalog with a fresh real price observation before dispatch"
e2e_refresh_azure_catalog_inputs
work_dir="$(mktemp -d)"
trap 'rm -rf "$work_dir"' EXIT
e2e_render capacity-catalog-azure.yaml.tmpl "$work_dir/capacity-catalog-azure.yaml"
kubectl apply -n "$E2E_NAMESPACE" -f "$work_dir/capacity-catalog-azure.yaml"
...
if [ -n "$allocation_id" ]; then
  ...
  while true; do
    ...
    if [ "$(date +%s)" -ge "$deadline" ]; then
      e2e_log "::warning:: allocation $allocation_id still in phase=$phase at the runtime ceiling - tools/e2e/verify-azure.sh and tools/e2e/teardown-azure.sh will still independently confirm/force cloud-side cleanup"
      break
    fi
    sleep 5
  done
