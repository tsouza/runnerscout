...
func TestGCPCredentialFilesIsolateRealTokenExchanges(t *testing.T) {
	...
	for _, name := range []string{"first", "second"} {
		...
		if err := os.WriteFile(file, []byte("changed-after-construction"), 0600); err != nil {
			t.Fatal(err)
		}
		service, err := compute.NewService(ctx, option.WithEndpoint(server.URL+"/compute/v1/"), option.WithHTTPClient(oauth2.NewClient(ctx, source)))
		if err != nil {
			t.Fatal(err)
		}
	...
	defer mu.Unlock()
	for _, name := range []string{"first", "second"} {
		if exchanges[name] != 1 || requests[name] != 2 {
			t.Fatal("credentials leaked or token reuse failed", exchanges, requests)
		}
	}
