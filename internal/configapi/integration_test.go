...
func TestRealKubernetesCRDSchemasAndConfigurationSnapshot(t *testing.T) {
	...
	if err != nil || len(files) != len(api.CRDKinds) {
		t.Fatalf("expected one generated CRD schema per api.CRDKinds entry (%d): %v", len(api.CRDKinds), err)
	}
	var created []string
	...
	verifyRealExamples(t, ctx, kc, dc, namespace+"-examples")
