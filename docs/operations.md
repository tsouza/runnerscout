...
and deadline. Setting `awsPriceRefresh`/`azurePriceRefresh` in the config file
(or, in CRD mode, `priceRefresh` on the `CapacityCatalog`) opts
...
an offering to the catalog remains a manual, file-based (or CRD-applied) change
either way.
