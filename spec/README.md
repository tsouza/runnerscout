...
| | | `post178_pre182` | **Violates** `FailureCauseVisible` (the real historical state between #178 and #182: JIT preserved, WireGuard/cloud-create/observe/delete still collapse) |
| | | `post_all_fixes` | Clean (shipped as of #182, not merely extrapolated: every external failure class preserves its own cause) |
...
| `ExternalCallBudget.tla` | every external (GitHub/cloud) call reachable from `runLeader`'s cancel-only, no-deadline ctx outside Step's own already-bounded per-allocation goroutine - nine real call sites, found across two audit passes (see `CallSites`'s own comment for why a third isn't ruled out) | `pre_fix` | **Violates** `NeverPermanentlyStuck` (any one stalling hangs reconciliation forever - reproduces a real incident: a leader pod idle, no log line, readyz 503, for 49+ minutes, identically on restart) |
| | | `partial_fix_witness` | **Violates** `NeverPermanentlyStuck` (the second real intermediate state this repo was actually in: seven sites fixed, `retry.go`'s `AttemptJobs`/`RerunFailedJobs` found only by an adversarial review of this module's own prior completeness claim) |
| | | `post_fix` | Clean (every known call site now wrapped in its own `externalCallBudget`/`githubStartupBudget` context) |
