...
Dependency/cache refresh: the user requested current third-party dependencies, all remaining development PRs merged through checks, and persistent Go CI caches. The working candidate uses Go 1.27.1/Kubernetes 1.37 and maintained Helm 3.22; compatibility exceptions are recorded in dependency-status.md. Fresh verification is required. Packaged installation and App-token exchange passed in the preceding Kubernetes 1.35 fixture run (12 checks, zero cleanup errors); the fixture does not validate App JWT signatures.
